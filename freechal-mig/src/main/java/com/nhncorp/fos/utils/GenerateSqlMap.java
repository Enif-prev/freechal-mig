...
public class GenerateSqlMap {
	/**
	 * domainClass, tableName 에 해당하는 select, insert, update 용 ibatis 맵을 생성하여 반환한다.<br>
	...
	/**
	 * domainClass, tableName 에 해당하는 select, insert, update 용 ibatis 맵을 생성하여 반환한다.
	 * @param domainClass
	 * @param tableName
	 * @param classAlias
	 * @return
	 ...
	 * @param classAlias
	 * @param tabSize 탭의 길이
	 * @param useMap select 시에 resultMap 을 사용할지 여부
	 * @return
	 */
	public static String generate(Class<?> domainClass, String tableName,
		...
		if (null != classAlias) {
			map.append("\t<typeAlias alias=\"").append(classAlias).append(
				"\" type=\"").append(domainClass.getName()).append("\"/>\n");
		} else {
	...
	private static void addResultMap(StringBuilder map, String classAlias,
		...
		map.append("\t<resultMap class=\"").append(classAlias).append(
			"\" id=\"").append(classAlias).append("Map\">\n");
		for (Field field : fields) {
			if (isSkipField(field)) {
				continue;
			}
			map.append("\t\t<result property=\"").append(field.getName()).append(
				"\"");
			addTabByFieldLength(map, tabSize, field.getName().length());
			map.append("column=\"").append(
				NodeNameConvertor.toUnderscore(field.getName())).append(
				"\"/>\n");
		}
	...
	private static void addSelectQuery(StringBuilder map, String classAlias,
		...
		map.append("\t<select id=\"get").append(
			NodeNameConvertor.toPascal(classAlias)).append(
			"\" parameterClass=\"").append(classAlias).append("\" resultMap=\"").append(
			classAlias).append("Map\">\n");
		map.append("\t\tSELECT ");
		int i = 0;
		for (Field field : fields) {
			if (isSkipField(field)) {
	...
	private static void addSelectQueryWithoutMap(StringBuilder map,
		...
		map.append("\t<select id=\"get").append(
			NodeNameConvertor.toPascal(classAlias)).append(
			"\" parameterClass=\"").append(classAlias).append(
			"\" resultClass=\"").append(classAlias).append("\">\n");
		map.append("\t\tSELECT ");
		int i = 0;
		for (Field field : fields) {
			if (isSkipField(field)) {
			...
			addSeparator(map, i);
			map.append(NodeNameConvertor.toUnderscore(field.getName())).append(
				" AS ").append(field.getName());
			i++;
		}
		map.append("\n\t\tFROM ").append(tableName).append("\n");
		map.append("\t</select>\n");
	...
	private static void addInsertQuery(StringBuilder map, String classAlias,
		String tableName, List<Field> fields) {
		map.append("\t<insert id=\"insert").append(
			NodeNameConvertor.toPascal(classAlias)).append(
			"\" parameterClass=\"").append(classAlias).append("\">\n");
	...
	private static void addUpdateQuery(StringBuilder map, String classAlias,
		String tableName, List<Field> fields) {
		map.append("\t<update id=\"update").append(
			NodeNameConvertor.toPascal(classAlias)).append(
			"\" parameterClass=\"").append(classAlias).append("\">\n");
